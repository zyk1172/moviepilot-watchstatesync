...
def _install_moviepilot_stubs():
    ...
    sdk_services = types.ModuleType("app.sdk.services")
    ...
    sdk_services.MediaServerHelper = MediaServerHelper
    sdk_logging.logger = log.logger
...
MODULE_PATH = Path(__file__).parents[3] / "plugins.v3" / "watchstatesync" / "__init__.py"
SPEC = importlib.util.spec_from_file_location("watchstatesync_v3_under_test", MODULE_PATH)
...
class WatchStateSyncTests(unittest.TestCase):
    ...
    def test_v3_runtime_state_is_instance_scoped(self):
        first = WATCHSTATESYNC.WatchStateSync()
        second = WATCHSTATESYNC.WatchStateSync()

        first._recent_writes["movie"] = 1.0
        first._plex_sessions["session"] = {"user": "alice"}
        first._jellyfin_auth_cache[("http://jellyfin", "alice")] = {"context": {}}
        first._plex_alert_listener = object()

        self.assertIsNot(first._lock, second._lock)
        self.assertIsNot(first._sync_lock, second._sync_lock)
        self.assertIsNot(first._poll_lock, second._poll_lock)
        self.assertEqual(second._recent_writes, {})
        self.assertEqual(second._plex_sessions, {})
        self.assertEqual(second._jellyfin_auth_cache, {})
